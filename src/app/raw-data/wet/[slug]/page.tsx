...
import { getWetGroupParams } from '@/lib/bioAiRoutes';

interface WetGroupPageProps {
  params: { slug: string };
}
...
export function generateStaticParams() {
  return getWetGroupParams();
}

export const dynamicParams = false;

export default function WetGroupPage({ params }: WetGroupPageProps) {
      ...
      {group.summary_points?.length ? (
        ...
        <section className="bio-grid">
          {group.entries.map((entry) => (
            <Link key={entry.slug} href={`/raw-data/wet/${group.slug}/${entry.slug}`} className="bio-feature-card">
