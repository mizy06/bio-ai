...
const dryGroups = bioAiDryGroups;
  ...
  return (
    <main className="bio-page">
      <section className="bio-hero card">
        ...
        <p className="bio-eyebrow">Bio-AI / Raw Data / Dry</p>
        <h1>Dry</h1>
        <p className="bio-lead">按干实验报告结构浏览平台分层、平台卡片与详情页。</p>
        <div className="bio-stat-row">
          <span className="bio-stat-pill">{dryGroups.length} layers</span>
          <span className="bio-stat-pill">{dryGroups.reduce((sum, group) => sum + group.entries.length, 0)} platforms</span>
      ...
      <section className="bio-grid">
        {dryGroups.map((group) => (
          <Link key={group.slug} href={`/raw-data/dry/${group.slug}`} className="bio-feature-card">
